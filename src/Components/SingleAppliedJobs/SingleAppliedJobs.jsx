...
const SingleAppliedJobs = ({ singleJob }) => {
  const { company_logo, company_name, job_title, remote_or_onsite, location, salary, id } = singleJob;
  const userEmail = localStorage.getItem('userEmail');
  const appliedJobs = JSON.parse(localStorage.getItem(`jobs_${userEmail}`)) || [];
  const isApplied = appliedJobs.some((job) => job.id === id);
    ...
    <div className='md:flex w-80 md:w-3/5 mx-auto border-2 rounded-3xl justify-between items-center p-4 bg-slate-200 mb-4'>
      <div className='md:flex gap-4 items-center'>
        <div className="image p-2">
          <img src={company_logo} className='w-36' alt="" />
        </div>
      ...
      <div className='flex flex-col gap-4'>
        <Link to={`/details/${id}`} className='custom-btn mt-4 w-full'>
          View Details
        </Link>
        {isApplied ? (
          <Link to={`/review/${id}`} className='custom-btn mt-4 w-full'>
            Review Details
          </Link>
        ) : (
          <Link to={`/uploadResume/${encodeURIComponent(JSON.stringify(singleJob))}`} className='custom-btn mt-4 w-full'>
            Apply Now
          </Link>
        )}
      </div>
